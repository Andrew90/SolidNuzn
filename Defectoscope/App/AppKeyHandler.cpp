...
namespace AppKeyHandler
{
	...
	template<class Head, class Tail>struct __btn__<Tlst<Head, Tail>>
	{
		void operator()()
		{
			SendMessage(mainWindow.toolBar.hWnd, TB_ENABLEBUTTON, Head::ID, MAKELONG(Head::state, 0));
			zprint("%d %d\n", Head::ID, Head::state);
			__btn__<Tail>()();
		}
	};
	...
	{
		return 0 != SendMessage(mainWindow.toolBar.hWnd, TB_ISBUTTONENABLED, id, 0);
	}
	...
	{
		TCommand c = {mainWindow.hWnd, WM_COMMAND, id, 0, mainWindow.toolBar.hWnd};
