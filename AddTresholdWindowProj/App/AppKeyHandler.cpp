...
namespace AppKeyHandler
{
	...
	template<class Head, class Tail>struct __btn__<Tlst<Head, Tail>>
	{
		void operator()()
		{
			SendMessage(App::MainWindowToolBarHWND(), TB_ENABLEBUTTON, Head::ID, MAKELONG(Head::state, 0));
			__btn__<Tail>()();
		}
	};
	...
	{
		return 0 != SendMessage(App::MainWindowToolBarHWND(), TB_ISBUTTONENABLED, id, 0);
	}
	...
	{
		TCommand c = {App::MainWindowHWND(), WM_COMMAND, id, 0, App::MainWindowToolBarHWND()};
