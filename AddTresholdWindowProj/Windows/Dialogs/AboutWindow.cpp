		...
		{			
			...
			for(int i = 0; i < dimention_of(about_text); ++i)
			{
				CreateWindow(L"static", about_text[i]
					, WS_VISIBLE | WS_CHILD
					, 10, dy + 3, width, 20, h, 0, (HINSTANCE)::GetModuleHandle(NULL), NULL
					);
				dy += 20;
			}
			...
			CreateWindow(L"button", L"Закрыть"
				...
				,offs, dy, 110, 30, h, (HMENU)IDOK, (HINSTANCE)::GetModuleHandle(NULL), NULL
				);
			RECT r;
...
void AboutWindow::Do(HWND hWnd)
{
	...
	DialogBoxIndirectParam((HINSTANCE)::GetModuleHandle(NULL), &d, hWnd, (DLGPROC)Proc, NULL);
    LocalFree((HLOCAL)p);
}